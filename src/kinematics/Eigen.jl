...
function expm(A::SymmetricTensor{2, 3, T, 6}) where T <: Number
  evals, evecs = eigen_sym33_unit(A)
  exp_evals = diagm(SymmetricTensor{2, 3, T, 6}, Tensors._map(exp, evals))
  return dot(evecs, dot(exp_evals, evecs')) |> symmetric
end
