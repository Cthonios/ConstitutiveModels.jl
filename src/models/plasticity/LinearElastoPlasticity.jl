...
function LinearElastoPlasticity(inputs::D, type::Type{ArrType}) where {D <: Dict{String, Any}, ArrType}
...
function isotropic_hardening_properties(model::LinearElastoPlasticity, props::V) where V <: AbstractArray{<:Number, 1}
  ...
  return @views props[props_start:props_start + n_props - 1]
end
...
function helmholtz_free_energy(model::LinearElastoPlasticity, props, F::Tensor{2, 3, T1, 9}, state_old::SVector{7, T2}) where {T1, T2}
  # extract props needed at this level
  μ              = props[2]
  elastic_props  = elastic_properties(model, props)
  iso_hard_props = isotropic_hardening_properties(model, props)
