...
$(TYPEDEF)
"""
struct Hencky <: AbstractHyperelasticity
end
...
$(TYPEDSIGNATURES)
"""
function helmholtz_free_energy(::Hencky, props, ∇u, θ)
    κ, μ = props[1], props[2]
...
$(TYPEDSIGNATURES)
"""
function pk2_stress(::Hencky, props, ∇u, θ)
    κ, μ = props[1], props[2]
...
$(TYPEDSIGNATURES)
"""
function pk1_stress(model::Hencky, props, ∇u, θ)
    F = ∇u + one(∇u)
    S = pk2_stress(model, props, ∇u, θ)
    return F ⋅ S
end
...
$(TYPEDSIGNATURES)
"""
function material_tangent(::Hencky, props, ∇u::Tensor{2, 3, T, 9}, θ) where T <: Number
    κ, μ = props[1], props[2]
