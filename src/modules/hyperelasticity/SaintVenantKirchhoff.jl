...
$(TYPEDEF)
"""
struct SaintVenantKirchhoff <: AbstractHyperelasticity
end
...
$(TYPEDSIGNATURES)
"""
function helmholtz_free_energy(::SaintVenantKirchhoff, props, ∇u, θ)
    λ, μ = props[1], props[2]
    F    = ∇u + one(∇u)
    C    = tdot(F)
...
function pk1_stress(::SaintVenantKirchhoff, props, ∇u, θ)
    λ, μ = props[1], props[2]
    F    = ∇u + one(∇u)
    C    = tdot(F)
    I    = one(C)
...
function material_tangent(::SaintVenantKirchhoff, props, ∇u, θ)
    λ, μ = props[1], props[2]
    F    = ∇u + one(typeof(∇u))
    C    = tdot(F)
    E    = 0.5 * (C - one(C))
