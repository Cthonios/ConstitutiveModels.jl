function _test_ad_equal_analytic_for_hyper_material_tangent(
    model, props, Z_old, Z_new, Δt, ∇us, θ;
    atol = 1e-10, rtol = 1e-10
)
    As_ad = map(x -> Tensors.gradient(
        z -> pk1_stress(model, props, Z_old, Z_new, Δt, z, θ), x
    ), ∇us)
    As_an = material_tangent.((model,), (props,), (Z_old,), (Z_new,), (Δt,), ∇us, (θ,))
    # display(map((x, y) -> x - y, As_ad, As_an))
    @test all(map((x, y) -> isapprox(x, y, atol = atol, rtol = rtol), As_ad, As_an))
end

function _test_ad_equal_analytic_for_hyper_pk1_stress(model, props, Z_old, Z_new, Δt, ∇us, θ)
    Ps_ad = map(x -> Tensors.gradient(
        z -> helmholtz_free_energy(model, props, Z_old, Z_new, Δt, z, θ), x
    ), ∇us)
    Ps_an = pk1_stress.((model,), (props,), (Z_old,), (Z_new,), (Δt,), ∇us, (θ,))
...
#     end
#     θ_func = t -> 0.0
#     history = simulate_material_point(
#         cauchy_stress, model, inputs, motion,
#         (λ_func,), θ_func, 1.0
#     )
#     ∇us = map(x -> x.kinematics, history)
#     σs = map(x -> x.material_output, history)

#     κ, C1, C2 = props[2], props[3], props[4]
...
function test_arruda_boyce()
    ...
    model = Hyperelastic(ArrudaBoyce())
...
function test_linear_elastic()
    ...
    model = Hyperelastic(LinearElasticity())
...
function test_mooney_rivlin_simple_shear(model, inputs)
    ...
    ψs_an = (C1 + C2) .* γs.^2
    ψs = helmholtz_free_energy.((model,), (props,), (Z_old,), (Z_new,), (Δt,), ∇us, (θ,))
    @test all(ψs_an .≈ ψs)
...
function test_mooney_rivlin_uniaxial_strain(model, inputs)
    ...
    ψs = helmholtz_free_energy.((model,), (props,), (zeros(0),), (zeros(0),), (0.0,), ∇us, (0.0,))
    @test all(ψs_an .≈ ψs)
...
function test_mooney_rivlin()
    ...
    model = Hyperelastic(MooneyRivlin())
    # test_mooney_rivlin_simple_shear(model, inputs)
...
function test_neohookean_pure_shear_strain(model, inputs)
    ...
    motion = PureShearStrain(λ_func)
    history = simulate_material_point(cauchy_stress, model, inputs, motion, 1.0)
    ∇us = map(x -> x.kinematics, history)
    σs = map(x -> x.material_output, history)
    λs = map(x -> λ_func(x.time), history)
...
function test_saint_venant_kirchoff()
    ...
    model = Hyperelastic(SaintVenantKirchhoff())
...
function test_seth_hill_uniaxial_strain(model, inputs)
    ...
    λs = map(x -> x[1, 1] + 1.0, ∇us)
    # λ, μ = props[1], props[2]
    κ, μ, m, n = props[2], props[3], props[4], props[5]
