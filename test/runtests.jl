...
function test_stress_eq(::SimpleShear, σs, σ_xx_ans, σ_yy_ans, σ_xy_ans)
...
function test_stress_eq(::M, σs, σ_xx_ans, σ_yy_ans) where M <: Union{UniaxialStrain, UniaxialStressDisplacementControl}
    @assert length(σs) == length(σ_xx_ans)
    @assert length(σs) == length(σ_yy_ans)
